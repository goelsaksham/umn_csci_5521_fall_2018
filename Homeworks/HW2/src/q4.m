...
kNN(1, train_X(1, :), train_X, train_y)
...
% most_misclassified_eight = find(projection_eight == max(projection_eight));
% projection_nine = nine_pc*LDA;
% most_misclassified_nine = find(projection_nine == min(projection_nine));
...
% plot([x_min, x_max], [classifier(2) * x_min / classifier(1), classifier(2) * x_max / classifier(1)],'-');
% hold on;
% % scatter(LDA);
% scatter(eight_pc(:, 1), eight_pc(:, 2), 'r*');
% scatter(nine_pc(:, 1), nine_pc(:, 2), 'b+');
% scatter(projection_eight * LDA(1), projection_eight * LDA(2), 'g');
% scatter(projection_nine * LDA(1), projection_nine * LDA(2), 'y');
