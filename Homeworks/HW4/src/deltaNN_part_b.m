function [dE_dU, dE_dV,dE_dW,E,z,y]=deltaNN(U,V,W,u,t);
% function [dE_dV,dE_dW,E,z,y]=deltaNN(V,W,u,t);
% compute derivative of square-norm Error funcional
% wrt V,W coeffs in a neural network with one hidden layer:
%  u -> U -> tanh -> x -> V -> tanh -> y -> W -> tanh -> z
%  x = input and is M x 1, V is N x M+1, W is P x N+1, 
% The input parameters are:
% - U (M X N + 1): 
% - V (N x O + 1): Weights between each input unit and each hidden unit 
% - W (O x P + 1): Weights between each hidden unit and each output unit
% - u (M x 1):input data, M is the number of features in one input datum
% - t (P x 1): True labels for the given datum
% The function should return:
% - dE_dU (M x N + 1): Updates for the weights in U
% - dE_dV (N x O + 1): Updates for the weights in V
% - dE_dW (O x P + 1): Updates for the weights in W
